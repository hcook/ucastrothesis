...
The \code{ReadsFromOuterCacheDataArray} trait defines the \code{readDataArray} method that generates the logic associated with this sub-flow,
...
Figure~\ref{fig:probetrait} shows the \code{EmitsInnerProbes} trait, which sends Probes to clients in order to prompt them to Release permissions on a cache block.
...
Figure~\ref{fig:readtrait} shows the \code{ReadsFromOuterCacheDataArray} trait, which sends requests to a local SRAM array contain the actual copies of the data blocks in order to read one.
...
and is an example of combining two traits that use the same messaging channel (i.e.,
 \code{AcceptsVoluntaryReleases} and \code{EmitsInnerProbes}).

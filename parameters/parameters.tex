...
as well as provide some insights into how we integrate CDEs with Chisel.
...
\end{description}
The function bound to a particular identifier can choose to ignore the \code{site} or  \code{here} argument and just return a constant value,
which provides the same functionality that would be found in a regular environment.
However, the use of more sophisticated parameter valuations based on querying \code{site} 
allows users to create chains of parameter dependencies and easily specialize deeply nested portions of our design.
...
The applicability to maps is obvious, and in fact Scala provides built-in functionality for converting between these data structures and first class partial function types.
Our CDEs are built by composing hierarchies of partial functions that map from an identifier (called \code{pname}) to a parameter value (or Scala object created using multiple parameters).
Our framework is very permissive about what Scala objects can be used as a \code{pname}, accepting \code{Any} Scala type.
...
Figure~\ref{fig:paramspf} provides some examples of making use of different types of identifiers.
We provide the \code{Field} base class to improve the syntax for looking up a particular \code{pname}, as well as
leverage the Scala type system to check that indentifiers from different projects to do not conflict.
...
class Location(x: Int, y: Int) extends Field[Int]

val config: PartialFunction[Any, Any] = {
  case Location(x, y) if x == 0 => 1
  case Location(x, y) if x != 0 => 2
}
  
\end{scala} 
\caption{Using partial functions for parameterization.}
\label{fig:paramspf}
\end{figure}
...
This cast can fail if the wrong type of object is provided, an inherent weakness of our dynamic scoping approach.
...
where modifing a parent creates a child with new bindings that
can override the parents, while in the absence of new bindings fall back on the parent's bindings.
...
val tileParameters = parameters.alter(
  (key, site) => { 
    case 'width' => 64 
  }) 
...
As discussed in the previous section, the sole additional feature of a CDE over a regular environment is a special object,
...
for deploying CDEs in a Chisel-based generator.
Beyond which paradigm to use, there are additionally a wide variety of design decisions that go into injecting parameters into a design,
and I would like to capture some of the tradeoffs we have explored while taping out chips using our SoC generator.
...
\subsection{Deeply Nested Bundles}

Store Data Queue example.

...
which is a critical featurs for design space exploration~\cite{shacham2011chip}.
...
Secondly, without a mechanism that allows designers to override parameter values within certain subsets of the module hierarchy, creating heterogenous systems where the same generator
...
This capability is at the heart of how we uniquify certain modules in a heterogenous design.
...
When we finally query \code{'coefficient'} inside of \code{Mult}, \code{site} resolves to an environment where the \code{'index'} key has since been defined (in the heterogenous case).
...
\caption{(1)-(6) are example module hierarchies modifications. 
...
Figure \ref{fig:both} illustrates this distinction.
...
In (2), a module with one parameter is appended to a 3-level hierarchy where one of its parents is instantiated twice ($\delta = 4$).
...
If were are appending $M$ rather than inserting it, $\rho$ will of course equal 0 because there cannot be an references to non-existant children.
...
However, as that generator becomed embedded within deeper and deeper generator hierarchies, the burden born by all the parent generators grows and grows, as captured by the $\pi$ term.
...
In cases where we are heterogenously adding an instance of a new module type, or homogenously adding an identical instantiation of an existing module type, no further work needs to be done.
...
While there are some disadvantages to using them as well, we discuss how these can be mitigated through good software engineering practices in the following sections.
...
\item stored in a container such as a value, variable, or data structure.
\item used as a parameter to a nother function or used as the return value from another function.
\end{itemize}
...
Our framework is very permissive about what Scala objects can be used as an identifier, accepting \code{Any} Scala type.
...
\centering
\begin{scala}
val p1 = Parameters.empty.alter("a" -> 1)
val a = p1[Int]("a")
...
where modifing a parent creates a child with new bindings that can override the parent's bindings,
...
\end{scala} 
\caption{Constraining parameter values. TODO }
\label{fig:constraints}
\end{figure}

TODO:
Expressions library that operates on fields.
Use simple expressions to describe relationship between parameters.
Figure~\ref{fig:site}
...
Constraints out: collect, dump, feed to solver.
Additional constraints in: for DSE.

Configurations represented via Scala source code. Define topDefs, knobs, top-level constraints.

\section{Parameterization Best Practices In Chisel}

The goal of this section is to distill some learned wisdom about effective software engineering strategies
...
CDEs for paramterization, but do compliment it.
...
class B(implicit p: Parameters) extends Module {
  ...
  val a32 = Module(new A(p.alter({"width" => 32})))
}

val b = Module(new B(Parameters.empty.alter({"width" => 16})))
\end{scala} 
\caption{Using Scala's implicit parameters to pass Parameter instances to Modules when no fields are overridden.}
\label{fig:implicit}
\end{figure}

Implicit parameters are a limited form of dynamic scoping that are another built-in feature of the Scala language.
...
While I will not discuss the resolution rules here, the salient feature is that it is based on only
a single value of a matching type being marked with the \code{implicit} keyword in lexical scope.

Implicit parameters are used to simplify Scala APIs by eliding parameters with which standard users do not need to concern themselves.
...
In cases where none of the Parameter fields are being altered (which is the common case), users do not have to explicitly pass the paramet
...
Scala supports multiple inheritance, wherein member defintions of multiple classes can be reused in the definition of a new class.
...
\end{scala} 
\caption{Two alternative approaches to representing sub-fields of inter-related parameters.}
\label{fig:fields}
\end{figure}
...
In the first approach, every independent parameter is represented by an individual Field definition.
Dependent parameters are then defined in traits after the parameters have been bound to local variables.
This approach plays nicely with our \code{Constraints} framework, as that relies on individual
However, it requires users to mixin the traits to gain access to
...
val l2toMCNetwork = Module(new TLNetwork()(params.alter({TLId => "L2toMC"}))) // Inject geography into context
val outerNetwork = Module(new TLNetwork()(params.alter({TLId => "Outermost"}))) // A different location
val icache = Module(new ICache()(params.alter({CacheName => "L1I"; TLId => "L2toMC"}))) // Can be multiply located
...
For each parameter that varies heterogenously across the design, we use \code{site}
to reference another parameter that abstractly describes the ``location'' of that parameter within the design.
Parameters which vary together are keyed off of the same geographic parameter.
...
it can be introducted by external users and injected into the design at the top-level or anywhere in between.
\code{site} is what allows us to decoupled the definition of the parameters bindings from the run-time evaluation of their values,
...
We have found that users are often discontent with varying each of these correlated parameters independenty.
When many locations are possible for each parameter, the replicated code to select amongst them can
...
can reference other georgraphic keys directly.
...
of hooking up our \code{Parameters} and \code{Constrait} abstractions to a design space exploration tool, called Jackhammer \cite{}.
...
Further work is required to automate the exploration process, and close the loop between feedback from one iteration of examing a
...
In the following chapters, we will move on to the specfics of how to express cache coherence protocols with hardware generators,
